...
const vehicle1 = createYukaCar({ maxSpeed: 30, minSpeed: 10, team: 'red', startPos: 1, model: CAR, track: TRACK, tyre: tyre(0) });
entityManager.add(vehicle1);

const vehicle2 = createYukaCar({ maxSpeed: 30, minSpeed: 10, team: 'blue', startPos: 2, model: CAR, track: TRACK, tyre: tyre(0) });
entityManager.add(vehicle2);

const vehicle3 = createYukaCar({ maxSpeed: 30, minSpeed: 10, team: 'red', startPos: 3, model: CAR, track: TRACK, tyre: tyre(0) });
entityManager.add(vehicle3);

const vehicle4 = createYukaCar({ maxSpeed: 30, minSpeed: 10, team: 'yellow', startPos: 4, model: CAR, track: TRACK, tyre: tyre(0) });
entityManager.add(vehicle4);

const vehicle5 = createYukaCar({ maxSpeed: 30, minSpeed: 10, team: 'blue', startPos: 5, model: CAR, track: TRACK, tyre: tyre(0) });
entityManager.add(vehicle5);

const vehicle6 = createYukaCar({ maxSpeed: 30, minSpeed: 10, team: 'lightblue', startPos: 6, model: CAR, track: TRACK, tyre: tyre(0) });
entityManager.add(vehicle6);

const vehicle7 = createYukaCar({ maxSpeed: 30, minSpeed: 10, team: 'lightblue', startPos: 7, model: CAR, track: TRACK, tyre: tyre(0) });
entityManager.add(vehicle7);

const vehicle8 = createYukaCar({ maxSpeed: 30, minSpeed: 10, team: 'yellow', startPos: 8, model: CAR, track: TRACK, tyre: tyre(0) });
entityManager.add(vehicle8);

const vehicle9 = createYukaCar({ maxSpeed: 30, minSpeed: 10, team: 'green', startPos: 9, model: CAR, track: TRACK, tyre: tyre(0) });
entityManager.add(vehicle9);

const vehicle10 = createYukaCar({ maxSpeed: 30, minSpeed: 10, team: 'green', startPos: 10, model: CAR, track: TRACK, tyre: tyre(0) });
entityManager.add(vehicle10);

const vehicle11 = createYukaCar({ maxSpeed: 30, minSpeed: 10, team: 'purple', startPos: 11, model: CAR, track: TRACK, tyre: tyre(1) });
entityManager.add(vehicle11);

const vehicle12 = createYukaCar({ maxSpeed: 30, minSpeed: 10, team: 'purple', startPos: 12, model: CAR, track: TRACK, tyre: tyre(1) });
entityManager.add(vehicle12);

const vehicle13 = createYukaCar({ maxSpeed: 30, minSpeed: 10, team: 'cyan', startPos: 13, model: CAR, track: TRACK, tyre: tyre(1) });
entityManager.add(vehicle13);

const vehicle14 = createYukaCar({ maxSpeed: 30, minSpeed: 10, team: 'orange', startPos: 14, model: CAR, track: TRACK, tyre: tyre(1) });
entityManager.add(vehicle14);

const vehicle15 = createYukaCar({ maxSpeed: 30, minSpeed: 10, team: 'pink', startPos: 15, model: CAR, track: TRACK, tyre: tyre(1) });
entityManager.add(vehicle15);

const vehicle16 = createYukaCar({ maxSpeed: 30, minSpeed: 10, team: 'cyan', startPos: 16, model: CAR, track: TRACK, tyre: tyre(1) });
entityManager.add(vehicle16);

const vehicle17 = createYukaCar({ maxSpeed: 30, minSpeed: 10, team: 'orange', startPos: 17, model: CAR, track: TRACK, tyre: tyre(2) });
entityManager.add(vehicle17);

const vehicle18 = createYukaCar({ maxSpeed: 30, minSpeed: 10, team: 'pink', startPos: 18, model: CAR, track: TRACK, tyre: tyre(2) });
entityManager.add(vehicle18);

const vehicle19 = createYukaCar({ maxSpeed: 30, minSpeed: 10, team: 'brown', startPos: 19, model: CAR, track: TRACK, tyre: tyre(2) });
entityManager.add(vehicle19);

const vehicle20 = createYukaCar({ maxSpeed: 30, minSpeed: 10, team: 'brown', startPos: 20, model: CAR, track: TRACK, tyre: tyre(2) });
...
function tyreDegradation(vehicle) {
  ...
  if (vehicle.inPit) {
    ...
    vehicle.maxSpeed = 27.5 - ((100 - tyreGrip * 3) / 100) + Math.random() * 0.5;
  } else if (tyreWear >= 40) {
    vehicle.maxSpeed = 27 - ((100 - tyreGrip * 3) / 100) + Math.random() * 0.5;
  } else if (tyreWear >= 35) {
    vehicle.maxSpeed = 26 - ((100 - tyreGrip * 3) / 100) + Math.random() * 0.5;
  } else {
    // Below 35% tyre life, apply further reductions gradually
    const reductionFactor = (35 - tyreWear) / 100; // Linearly reduce the max speed
    vehicle.maxSpeed = 25 - 8 - ((100 - tyreGrip) / 100) * reductionFactor; // Linearly reduce from 25 to 12 as tyre life decreases from 35 to 0
  }
}
...
function createYukaCar({ maxSpeed, minSpeed, team, startPos, model, track, tyre }) {
  ...
  const onPathBehavior = new YUKA.OnPathBehavior(vehicle.path); // can change radius and predictor factor dont know how they work yet 0.1 and 1 are default
